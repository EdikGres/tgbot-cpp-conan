...
int DBHandler::add_user(int64_t tg_id, const string &name, int cur_menu, int access_level) {
    if (name.length() > 120)
        return 1;
    ...
    if (mysql_num_rows(result) == 0) {
        sprintf(buffer, "INSERT INTO users VALUES (NULL, %ld, '%s', %d, %d)", tg_id, name.c_str(), cur_menu,
                access_level);
        if (mysql_query(con, buffer)) {
            error_handle();
            return 4;
